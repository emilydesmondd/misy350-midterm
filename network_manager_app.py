    ...
    with tab2:
        ...
        for person in advisors:
            ...
            with st.container(border=True):
                selected_item = st.radio("Select an item", edited_connections , label_visibility="collapsed")
            ...
            edit_name = st.text_input("Full Name", value = advisor_tochange['full_name'], 
                                key=f"edit_name_{advisor_tochange['id']}")
            edit_email = st.text_input("Email", value= advisor_tochange['email'], 
                                    key = f"edit_email_{advisor_tochange['id']}")
            edit_company = st.text_input("Company", value= advisor_tochange['company'],
                                    key = f"edit_company_{advisor_tochange['id']}")
            edit_position = st.text_input("Position", value= advisor_tochange['position'],
                                    key = f"edit_position_{advisor_tochange['id']}")

            update_btn = st.button("Update Connection",key="btn_update",use_container_width=True,type="primary")
            if update_btn:
